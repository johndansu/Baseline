...
|---|---|---|---|---|
| 01.1 | Hardcoded secrets, tokens, or API keys in codebase | PASS | `frontend-nodejs/public/js/supabase-config.js` uses placeholder anon key; API keys loaded from env in `internal/api/config.go` | Keep secret scanning in CI and block commits with real keys. |
